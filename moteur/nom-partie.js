...
// Le nom complet d'une partie — voir l'en-tete du fichier. Toutes les
// informations necessaires sont deja dans `donnees` (les champs de haut
// niveau ecrits par moteur.arbreVersDonnees) : pas besoin de rejouer
// toute la partie pour ca, seulement de suivre `is_origin` jusqu'au bout
// (moteur.cheminOrigineDuFichier) pour connaitre sa profondeur — le
// numero de tour de KAAWA (`display_turn = (history_index+1)//2`).
function nomDeFichierKAAWA(donnees) {
  const p1 = nomJoueurNettoye(donnees.Players.P1_black);
  const p2 = nomJoueurNettoye(donnees.Players.P2_white);
  const profondeurOrigine = cheminOrigineDuFichier(donnees.Tree).length;
  const tour = Math.max(1, Math.floor((profondeurOrigine + 1) / 2));
  // KAAWA : " (en cours)" tant que le vainqueur n'est pas un vrai nom
  // (litteralement "None" sinon) — meme si un statut de fin existe deja
  // (une nulle sans vainqueur designe, par exemple).
  const finTexte = donnees.Winner && donnees.Winner !== 'None' ? ` ${donnees.Winner}` : ' (en cours)';
  const base = `${donnees.Date}, ${donnees.Event}, ${donnees.VariantName}, ${p1}-${p2}, -${donnees.Eject.P1}-${donnees.Eject.P2}tr${tour}${finTexte}, ${donnees.Term}`;
  return possedeUneBranche(donnees.Tree) ? `Br_${base}` : base;
}
